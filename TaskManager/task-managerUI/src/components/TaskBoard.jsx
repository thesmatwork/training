...
// Tasks are kept separately per logged-in user (keyed by email) so that
// two different mock accounts never see each other's tasks.
const mockTasksByUser = new Map(); // email -> { tasks: [...], nextId: number }
...
async function apiRequest(path, options, token, email) {
  if (MOCK_MODE) {
    try {
      return await mockApiRequest(path, options, email);
    } catch (err) {
...
export default function TaskBoard({ token, email, onLogout }) {
  ...
  const loadTasks = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const data = await apiRequest("/tasks", undefined, token, email);
  ...
  const handleDelete = async (task) => {
    ...
    try {
      await apiRequest(`/tasks/${task.id}`, { method: "DELETE" }, token, email);
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <header style={{ marginBottom: 28, display: "center", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
            {email && (
              <div style={{ fontSize: 12.5, color: "#8A8371", marginBottom: 6, wordBreak: "break-all" }}>{email}</div>
            )}
            <button
              onClick={() => onLogout()}
              style={{
